...
export function convertToCSV<TData>(data: TData[], columns: ColumnDef<TData, any>[], includeHeaders = true): string {
    ...
    const exportableColumns = columns.filter((column) => column.id !== 'select' && column.id !== 'actions' && !column.id?.startsWith('_'));
    ...
    const rows = data.map((row) => {
        return exportableColumns
            .map((column) => {
                ...
                return stringValue.includes(',') || stringValue.includes('\n') || stringValue.includes('"') ? `"${escaped}"` : escaped;
            })
            .join(',');
    });
...
export function downloadCSV<TData>(data: TData[], columns: ColumnDef<TData, any>[], filename = 'data-export.csv'): void {
